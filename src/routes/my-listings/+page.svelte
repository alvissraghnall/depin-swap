<script lang="ts">
    ...
    try {
        ...
        alert(err instanceof Error ? err.message : 'An error occurred while removing the listing. Please try again.');
    } finally {
    ...
    <div class="flex flex-col gap-8">
        ...
        {#if !$isConnected}
            <div class="flex flex-col items-center justify-center py-16 gap-4">
                ...
                <p class="text-xl font-medium text-gray-600 dark:text-gray-400">Connect your wallet to view your listings</p>
                <button
                    onclick={handleConnectWallet}
                    class="flex min-w-[120px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em] transition-opacity hover:opacity-80"
                >
                    Connect Wallet
                </button>
            </div>
