    ...
    <div class="flex flex-col gap-8">
      ...
      <div class="grid grid-cols-1 gap-12 lg:grid-cols-3">
        ...
        <div class="lg:col-span-1 flex justify-center items-start">
          <div class="flex h-64 w-64 items-center justify-center rounded-xl bg-white/5 border border-white/10 shadow-sm">
            <span class="material-symbols-outlined text-primary text-9xl">
        ...
        <div class="flex flex-col gap-8 lg:col-span-2">
          ...
          <div class="flex flex-col gap-4">
            <div class="flex flex-wrap gap-3">
              <div class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full {getTypeColor(listing.type)}">
                <p class="text-sm font-medium">{listing.type}</p>
              </div>
              <div class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-primary/20 text-primary">
                <p class="text-sm font-medium">Available</p>
              </div>
              <div class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-white/10 border border-white/20 px-4">
                <p class="text-sm font-medium">Verified Lister</p>
              </div>
            </div>
            <div>
              <h1 class="text-4xl font-bold tracking-tighter text-white">{listing.title}</h1>
              <p class="mt-2 text-white/60">High-performance {listing.type.toLowerCase()} resource for your needs.</p>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-white/10 bg-white/5 p-6">
            ...
            <div class="mt-4 flex items-center justify-between border-t border-white/10 pt-4">
              <div class="flex items-center gap-3">
                <div class="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-12 bg-white/10 flex items-center justify-center">
              ...
              <button class="flex items-center justify-center gap-2 rounded-lg h-10 px-4 text-sm font-medium border border-white/10 bg-white/5 hover:bg-white/10">
          ...
          <!-- Status Message -->
          {#if statusMessage}
            <div class="rounded-lg bg-blue-500/10 border border-blue-500/20 px-4 py-3 text-sm text-blue-400">
              {statusMessage}
            </div>
          {/if}
          ...
          <div class="flex flex-col gap-4 sm:flex-row">
            <button
              ...
              class="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-3 px-6 text-base font-bold text-background-dark shadow-lg shadow-primary/30 transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {#if isProcessing}
            ...
            <button
              ...
              class="flex w-full items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/5 py-3 px-6 text-base font-bold text-white transition hover:bg-white/10 sm:w-auto"
          ...
          <div class="border-t border-white/10 pt-6">
            <details class="group" ontoggle={(e) => showTechnicalDetails = (e.target as HTMLDetailsElement)?.open}>
              ...
              <div class="mt-4 grid grid-cols-1 gap-x-4 gap-y-2 text-sm sm:grid-cols-2">
                ...
                <div class="flex justify-between border-b border-white/10 py-2 sm:border-none">
                  ...
                  <span class="text-white">{new Date(listing.createdAt as unknown as string).toLocaleDateString()}</span>
                </div>
